...
function visible(p) {
  return inArea(p, AREA[DESK_ROT]) || inArea(p, AREA.front) || isChip(p);
}
...
function renderCards() {
  ...
  for (var i = 0; i < cards.length; i++) {
    ...
    if (elt == undefined) {
      ...
      if (cards[i].rank != 'joker') {
        if (isChip(cards[i])) {
          ...
          elt.innerHTML =
            '<div style="margin:5px"><font color=' +
            COLORS[cards[i].suit] +
            '>' +
            SYMBOLS[cards[i].suit] +
            '<br>' +
    ...
    var moving =
      (mouseDownInfo.state == 'multimove' || mouseDownInfo.state == 'single') &&
      issel;
    var stopped =
      Math.abs(cards[i].targ.x - cards[i].x) < 2 &&
      Math.abs(cards[i].targ.y - cards[i].y) < 2;
  ...
  if (mouseDownInfo.state == 'multiselect') {
    ...
    if (cardLength > 0 && chipAmount > 0) {
      ...
      selbox.innerHTML = chipAmount + ' chips';
    } else if (cardLength > 0) {
      selbox.innerHTML = cardLength + ' cards';
    }
...
function cardMain(commitCallback) {
  ...
  var handhold = function() {
    ...
    for (var i = 0; i < mouseSel.cards.length; i++) {
      ...
      mouseSel.cards[i].targ.x = Math.min(
        Math.max(mouseSel.cards[i].targ.x, CARD_WIDTH / 2),
        WIDTH - CARD_WIDTH / 2
      );
      mouseSel.cards[i].targ.y = Math.min(
        Math.max(mouseSel.cards[i].targ.y, CARD_HEIGHT / 2),
